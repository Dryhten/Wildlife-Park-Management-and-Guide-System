<template>
    <div id="booking-management">
      ...
      <el-table :data="tableData" style="width: 100%">
        <el-table-column prop="id" label="预约ID" width="80" fixed></el-table-column>
        ...
        <el-table-column label="园区ID" width="120">
          <template slot-scope="scope">
            <el-input
              v-if="isshow[scope.$index]"
              ...
              size="mini"
            ></el-input>
            <span v-else>{{ scope.row.parkId }}</span>
          </template>
        </el-table-column>
        <el-table-column label="园区名称" width="150">
          <template slot-scope="scope">
      ...
      <!-- 新增对话框 -->
      <el-dialog title="新增预约记录" :visible.sync="addDialogVisible" width="50%">
        <el-form
          ref="addForm"
          :model="addForm"
          :rules="addFormRules"
  ...
  export default {
    data() {
      return {
        ...
        statusOptions: ['Pending', 'Playing','Completed', 'Cancelled'] // 根据后端状态选项进行调整
      };
    },
    computed: {
      // 判断是否为系统管理员
    ...
    methods: {
      getPageList() {
        this.$axios
          .get("/api/admin/bookings/list", { params: this.searchForm })
          .then((res) => {
      ...
      handledownload() {
        window.open("/api/admin/bookings/download");
      },
      handleEdit(index, row) {
        this.$set(this.isshow, index, true);
        
        // 如果不是系统管理员，只能编辑状态字段，不能修改用户ID和园区ID
        if (!this.isSystemAdmin) {
          ...
          const originalParkId = this.tableData[index].parkId;
          ...
          this.$watch(`tableData.${index}.parkId`, (newVal) => {
            if (!this.isSystemAdmin && newVal !== originalParkId) {
              this.$set(this.tableData[index], 'parkId', originalParkId);
              Message.warning('只有系统管理员才能修改园区ID');
            }
          });
        }
      ...
      handleCheck(index, row) {
        ...
        if (
          ...
          updatedRow.parkId === originalRow.parkId &&
        ...
        this.$axios
          .post("/api/admin/bookings/update", updatedRow)
          .then(() => {
      ...
      handleDelete(index, row) {
        MessageBox.confirm(
          ...
          this.$axios
            .post("/api/admin/bookings/delete", row)
            .then(() => {
      ...
      handleSubmit() {
        this.$refs.addForm.validate((valid) => {
          if (valid) {
            ...
            this.$axios
              .post("/api/admin/bookings/add", formData)
              .then(() => {
